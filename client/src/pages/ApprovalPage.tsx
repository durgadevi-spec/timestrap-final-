...
export default function ApprovalPage({ user }: { user: User }) {
  ...
  const filteredSubmissions = useMemo(() => {
    return uniqueTimeEntries.filter(s => {
      ...
      <div className="space-y-3">
        {filteredSubmissions.length === 0 ? (
          ...
          filteredSubmissions.map((entry) => {
            ...
            return (
              <Card key={entry.id} className="bg-slate-800/40 border-blue-500/10 p-4 transition-all hover:bg-slate-800/60">
                {/* Header: Name, Status and TIME + COMPLETION */}
